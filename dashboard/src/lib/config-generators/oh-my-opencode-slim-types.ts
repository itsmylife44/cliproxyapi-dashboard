 ...
 *
 * Based on oh-my-opencode-slim v0.9.14
...
/** Advanced agents supported by upstream slim but not auto-generated by the dashboard grid */
 ...
 * Can contain standard agents plus any explicitly configured advanced agents.
 */
export interface SlimPreset {
  ...
  // Allow explicit advanced agent configs
  observer?: SlimAgentConfig;
  councillor?: SlimAgentConfig;
  "council-master"?: SlimAgentConfig;
  // Allow any other advanced agent keys
  [key: string]: SlimAgentConfig | undefined;
}
...
export interface SlimInterviewConfig {
  ...
  port?: number;
  /** Enable dashboard integration mode */
  dashboard?: boolean;
}
...
export function validateSlimConfig(raw: unknown): OhMyOpenCodeSlimFullConfig {
  ...
  if (obj.presets && typeof obj.presets === "object" && !Array.isArray(obj.presets)) {
    ...
    for (const [presetName, presetVal] of Object.entries(presetsObj)) {
      ...
      for (const [agentName, agentVal] of Object.entries(pObj)) {
        if (typeof agentName !== "string" || agentName.length > 128) continue;
  ...
  // disabled_agents — bounded
  if (Array.isArray(obj.disabled_agents)) {
    const items = obj.disabled_agents.slice(0, 50).filter((v): v is string => typeof v === "string" && v.length <= 256);
    if (items.length > 0) result.disabled_agents = items;
  }
  ...
  if (obj.fallback && typeof obj.fallback === "object" && !Array.isArray(obj.fallback)) {
    ...
    if (fbObj.chains && typeof fbObj.chains === "object" && !Array.isArray(fbObj.chains)) {
      ...
      for (const [agent, arr] of Object.entries(chainsObj)) {
        if (typeof agent !== "string" || agent.length > 128) continue;
  ...
  if (obj.interview && typeof obj.interview === "object" && !Array.isArray(obj.interview)) {
    ...
    if (typeof intObj.dashboard === "boolean") {
      interview.dashboard = intObj.dashboard;
    }
