...
|----------|-------------|
| `CLIPROXYAPI_MANAGEMENT_URL` | Existing CLIProxyAPI management endpoint, ending in `/v0/management`; it must be reachable from inside the dashboard container |
| `MANAGEMENT_API_KEY` | Existing CLIProxyAPI management key/password |
| `API_URL` | Public URL clients should use for the existing CLIProxyAPI API |
